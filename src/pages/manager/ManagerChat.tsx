          ...
          {filteredContacts.map(contact => (
            ...
            >
              <div style={{ position: 'relative', marginRight: '16px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: contact.type === 'group' ? 'rgba(238, 77, 45, 0.1)' : 'rgba(238, 77, 45, 0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2563EB', fontSize: '20px', fontWeight: 600 }}>
              ...
              <div style={{ flex: 1, overflow: 'hidden' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: activeChatId === contact.id ? '#2563EB' : '#94a3b8', fontWeight: activeChatId === contact.id || contact.unreadCount > 0 ? 600 : 400 }}>{contact.lastMessageTime}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {contact.unreadCount > 0 && (
                    <span style={{ background: '#2563EB', color: 'white', borderRadius: '50%', width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, marginLeft: '8px' }}>
                      {contact.unreadCount}
                    </span>
                  )}
                </div>
            ...
            {document.getElementById('mgr-topbar-center') && createPortal(
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', paddingLeft: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: activeContact.type === 'group' ? 'rgba(238, 77, 45, 0.1)' : 'rgba(238, 77, 45, 0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2563EB', fontSize: '16px', fontWeight: 600 }}>
                    ...
                    <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start', marginBottom: '8px' }}>
                      <div style={{ 
                        ...
                        {activeContact.type === 'group' && !isMe && (
                          <div style={{ fontSize: '12px', fontWeight: 600, color: '#2563EB', marginBottom: '2px' }}>{msg.sender_name}</div>
                        )}
                        {msg.attachment && (
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px', marginTop: '4px' }}>
                          ...
                          {isMe && (
                            <span style={{ color: msg.status === 'read' ? '#2563EB' : 'rgba(238, 77, 45, 0.4)' }}>
            ...
            <div style={{ padding: '16px 24px', background: '#f0f2f5', borderTop: '1px solid #e5e5e5', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', position: 'relative' }}>
                ...
                <form onSubmit={handleSendMessage} style={{ flex: 1, display: 'flex', alignItems: 'center', background: '#ffffff', borderRadius: '24px', padding: '8px 16px', border: 'none' }}>
                  ...
                  />
                  <button type="submit" disabled={!inputText.trim() && !attachmentBase64} style={{ background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', color: (inputText.trim() || attachmentBase64) ? '#2563EB' : '#94a3b8', padding: 0 }}>
                    <Send size={20} />
                  </button>
                </form>
              </div>
